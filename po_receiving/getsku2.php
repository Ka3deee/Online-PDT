...
if(isset($_REQUEST['sku'])){
    ...
    if($hasduplicate){
        $Getdupli_query = "select Ponumb,Inumber,idescr,Expqty,istdpk,rcvqty from po_transfers  where 	Inumber = '$sku' and refno = '$refno'";					
        ...
        if($result1->rowCount() > 0){
           ...
           while($rowd = $result1->fetch(PDO::FETCH_ASSOC)){
			   $qty_rcv = $rowd['rcvqty'];
			   if ((float)$qty_rcv != (float)$rowd['Expqty']){
				   ?><tr>
            ...
            if($exceed_limit){
               ?>
                <div class="col-xs-6">
                    ...
                    <input value = "1"   class="form-control" id="qty" type="number">
                </div>
               <?php 
            }else{
                ?>
