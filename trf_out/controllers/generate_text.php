...
function generateTextTrfOutPickUpc($query, $conn) {
    ...
    while ($row = $tableData->fetch_assoc()) {
        $content .= $row['whmove'] . ',' . $row['inumbr'] . ',' . $row['whmumr'] . ',' . $row['whmvqt'] . ',' . $row['whmvqr'] . ',' . $row['iupc'] . ',' . $row['idescr'] . ',' . $row['strnum'] . ',' . $row['rcvqty'] . ',' . $row['expqty'] . ',' . $row['rexpday'] . "\n";
    }
...
$pickUPCQuery = "SELECT * FROM pickupc";
...
$usersQuery = "SELECT * FROM users WHERE active = 1";
...
header("Content-Disposition: attachment; filename=TrfReleasingMasterData_{$currentDate}.txt");
