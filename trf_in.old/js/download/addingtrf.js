        ...
        $.each(values, function( index, value ) {
            ...
            $.ajax({
                ...
                success : function(response)
                {
                    ...
                    progressbar_value = Math.round(q / values.length * 100);
                    //console.log('Progress bar value : ' + progressbar_value + ' Que : '+ q);
                    $('#download_progressbar').css("width",progressbar_value+"%");
                    $('#download_progressbar').attr('aria-valuenow',progressbar_value);
                
                    if(progressbar_value == 100){
                        $('#download_progressbar').html(progressbar_value+'% FINISHED');
                    }else{
                        $('#download_progressbar').html(progressbar_value+'%');
                    }
